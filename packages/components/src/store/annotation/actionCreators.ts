...
// 更新tag信息
export function UpdateTagList(tagConfigList: OneTag[]) {
  return {
    type: ANNOTATION_ACTIONS.UPDATE_TAG_LIST,
    payload: {
      tagConfigList,
    },
  };
...
// 更新多工具基本配置
export function UpdateToolsBasicConfig(toolsBasicConfig: BasicConfig[]) {
  return {
    type: ANNOTATION_ACTIONS.UPDATE_TOOLS_CONFIG,
    payload: {
      toolsBasicConfig,
    },
  };
...
export function InitTaskData({
  ...
  if (toolsBasicConfig) {
    // 更新所配置工具配置
    tasks.push(UpdateToolsBasicConfig(toolsBasicConfig));
    const tmpStep = initToolName
      ? toolsBasicConfig.filter((item: { tool: any }) => {
 ...
 * 更新当前工具名称
 */
export const ChangeCurrentTool = (toolName: string) => (dispatch: Function) => {
  dispatch({
    type: ANNOTATION_ACTIONS.UPDATE_CURRENT_TOOLNAME,
    payload: {
      toolName,
    },
  });
 ...
 * @param submitType
 */
export const ToSaveFileData = (submitType: ESubmitType) => (dispatch: any) =>
  [
    dispatch({ type: ANNOTATION_ACTIONS.SAVE_LABELRESULT }),
    dispatch({ type: ANNOTATION_ACTIONS.SAVE_LABEL_FILE_DATA, payload: { submitType } }),
  ];
