...
class LineToolOperation extends BasicToolOperation {
  ...
  constructor(props: ILineOperationProps) {
    ...
    this.setConfig(props.config as LineToolConfig);
    this.prevAxis = {
      x: 0,
      y: 0,
    };
  ...
  get enableOutOfTarget() {
    return this.config.drawOutSideTarget ?? this.config.drawOutsideTarget ?? this.config.enableOutOfTarget;
  }
  ...
  public drawLines = () => {
    try {
      ...
      lineList.forEach((line: ILine) => {
        ...
        if (line.pointList && line.isVisible) {
  ...
  public drawText(coord: ICoordinate, text: string, color: string, lineWidth?: number) {
    if (this.ctx) {
      ...
      if (lineWidth) {
        ...
        this.ctx.fillText(text, coord.x - LINE_ORDER_OFFSET.x - 10, coord.y - LINE_ORDER_OFFSET.y + 10);
      }
  ...
  /**
   * 通过pointList中的点设置选中区域
   */

  public setActiveAreaByPoint(point: ICoordinate, outsideCancel: boolean = false) {
    // const bounding = this.canvas.getBoundingClientRect();
    const coord = this.coordUtils.getRenderCoord(point);
  ...
  public setConfig(config: LineToolConfig) {
